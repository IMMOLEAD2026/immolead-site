...
function indicateurs(cp, annee, anneeRef, now, ref) {
  ...
  // Tendance : DVF est publié avec du retard, le dernier millésime peut être
  // incomplet. On compare donc les deux années sur le MÊME nombre de mois,
  // sinon la baisse de volume serait un artefact de fichier partiel.
  const moisMax = Math.max.apply(null, now.map(x => x.date.getMonth()));   // 0-11
  out.mois_couverts = moisMax + 1;

  const nowCadre = now.filter(x => x.date.getMonth() <= moisMax);
  const refCadre = ref.filter(x => x.date.getMonth() <= moisMax);

  if (nowCadre.length >= VOL_MIN_TENDANCE && refCadre.length >= VOL_MIN_TENDANCE) {
    const vRef = refCadre.map(x => x.valeur).sort((a, b) => a - b);
    const vNow = nowCadre.map(x => x.valeur).sort((a, b) => a - b);
    const mRef = quantile(vRef, 0.5), mNow = quantile(vNow, 0.5);
    if (mRef && mNow) {
      out.annee_ref = anneeRef;
      out.evolution_prix_pct   = Math.round((mNow - mRef) / mRef * 100);
      out.evolution_volume_pct = Math.round((nowCadre.length - refCadre.length) / refCadre.length * 100);
      if (moisMax < 11) out.periode_partielle = true;   // millésime incomplet
    }
  }
